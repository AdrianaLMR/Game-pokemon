...
musicControl.addEventListener('click', (event) => {
    ...
    event.target.src = `${event.target.src}`.includes('on.png')
        ? "../pokemon-game-base-main/assets/icons/off.png"
        : "../pokemon-game-base-main/assets/icons/on.png";
...
const interval = setInterval(() => {
    if (currentCount <= 0) {
        game.style.backgroundImage = "url('../pokemon-game-base-main/assets/game-over.jpg')"
...
function finishGame() {
    if (findCharmander && findPikachu && findZubat) {
        ...
        const timeOut = setTimeout(() => {
            game.style.backgroundImage = "url('../pokemon-game-base-main/assets/winner.jpg')";
...
function verifyLookpokemon(to) {
    ...
    if ((getTopPosition() >= 474 && getTopPosition() <= 594) && (getRightPosition() <= 138 && getRightPosition() >= 42)) {
        if (!findZubat) { // Verifica se o Zubat já foi encontrado
    ...
    switch (event.code) {
        case "ArrowLeft":
            if (getRightPosition() < 770) {
                ...
                ash.src = "../pokemon-game-base-main/assets/left.png"
            }
            break;
        ...
        case "ArrowRight":
            if (getRightPosition() > 2) {
                ash.style.right = `${getRightPosition() - 8}px`
                ash.src = "../pokemon-game-base-main/assets/right.png"
            }
            break;
        ...
        case "ArrowDown":
            if (getTopPosition() < 625) {
                ...
                ash.src = "../pokemon-game-base-main/assets/front.png"
            }
            break;
        ...
        case "ArrowUp":
            if (getTopPosition() > 2) {
                ash.style.top = `${getTopPosition() - 8}px`
                ash.src = "../pokemon-game-base-main/assets/back.png"
            }
            break;
